...
const setUserStateSql = async (userId, state) => {
  let setUserStateSql = `update user set state=${state} where userId='${userId}';`;
    await connection.query(setUserStateSql, (err, rows, fields) => {
      if (err) throw err;
    });
}
const setAllUserStateSql = async (state) => {
  let setUserStateSql = `update user set state=${state};`;
    await connection.query(setUserStateSql, (err, rows, fields) => {
      if (err) throw err;
    });
}
const setRoomStateSql = async (roomId, state) => {
  let setRoomStateSql = `update rooms set state=${state} where id = '${roomId}';`;
    await connection.query(setRoomStateSql, (err, rows, fields) => {
      if (err) throw err;
    });
}
const setPointAndSendThanksMessage = async (WebhookEventObject, userId, point) => {
...
const textReply = async (WebhookEventObject) => {
  const userId = WebhookEventObject.source.userId;
    let state_A202 = 0;
    let getRoomStatusSql = `select state from rooms where name='A202';`;
    await connection.query(getRoomStatusSql, (err, rows, fields) => {
      if (err) throw err;
      console.log('state_A202', rows[0].state); 
      state_A202 = rows[0].state;
    });
    let user_state = 0;
    let getUserStatusSql = `select state from user where userId ='${userId}';`;
    await connection.query(getUserStatusSql, (err, rows, fields) => {
      if (err) throw err;
      console.log('user_state', rows[0].state); 
  ...
  if (WebhookEventObject.message.text === '送信') {
    ...
    switch(state_A202) {
      ...
      case 1:
      case 2:
        await setUserStateSql(userId, 1);
    ...
    console.log('1state_A202', state_A202);
    await setRoomStateSql(1, 3);
    console.log('2state_A202', state_A202);
    await setRoomStateSql(1, 1);
    console.log('3state_A202', state_A202);
    switch(state_A202) {
      case 0:
        replyOnlyTextMessage(WebhookEventObject, '現在はA202は消灯されているまたは授業中なので消さなくて大丈夫です');
        break;
      case 1: 
        ...
        await setUserStateSql(userId, 2);
        SendMessageObject = [{
          "type": "template",
    ...
    switch(state_A202) {
      ...
      case 1:
        ...
        await setUserStateSql(userId, 3);
        setRoomStateSql(1, 2);
        break;
      case 2:
        replyOnlyTextMessage(WebhookEventObject,'現在電気を消してくれた方がいて、点灯状況の確認をしております。2分後にまた送信してください');
    ...
    switch(state_A202) {
      ...
      case 1: 
      case 2:
        await setUserStateSql(userId, 0);
        replyOnlyTextMessage(WebhookEventObject, 'ありがとうございました');
        break;
      default:
        console.log('room status error');
